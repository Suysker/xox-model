...
- SaaS Agent architecture should be a harness, not a flat function-calling catalog. Keep model runtimes behind adapters, route every run through a kernel, project only task-relevant tools, and let approval, memory isolation, audit, and business execution live in the application layer.
...
- Claude Code-style artifacts such as project memory, skills, hooks, subagents, transcripts, and checkpoints are design patterns, not SaaS enforcement by themselves. For business operations, map them to server-owned prompts, tool projection, deterministic policy, editable confirmations, audit logs, and domain-service execution.
- For xox-model, the target Agent OS architecture is a Lean Product Harness, not an OpenClaw-style control plane. Keep the core to Conversation Store, Context Pack, Tool Projector, Runtime Adapter, and Approval Executor; defer gateway, subagents, MCP host, and distributed workers until product demand is real.
- Implement tool projection at the runtime boundary, not only in docs. Provider adapters should receive the projected tool list as input, and API tests should assert representative prompts expose task-relevant tools while excluding unrelated high-risk tools and account actions.
