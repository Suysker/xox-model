...
- Obsolete local harness helper files are intentionally removed: `agent-run-engine.ts`, `turn-resolver.ts`, `agent-action-runtime.ts`, `context-engine/index.ts`, the former top-level `agentic-os-adapter.ts`, `runtime/provider-failover-policy.ts`, and provider runtime duplicates now owned by Agentic OS runtime packages.
