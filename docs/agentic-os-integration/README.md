...
- xox tool-call supervision and runtime event payloads now consume `@agentic-os/core` `createToolSupervisorCall()`, `shouldBlockToolCallOutsideInventory()`, `buildToolSupervisorFailureObservation()`, `summarizeToolSupervisorObservation()`, and `buildTool*Event()` helpers. `apps/api/src/agent/tool-runtime/tool-call-supervisor.ts` now only maps xox planner steps/results and persists product run events; `tool-execution-events.ts` only adapts core events to `@xox/contracts` while preserving Chinese product summaries.
