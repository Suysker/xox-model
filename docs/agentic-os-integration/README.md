...
- xox `planner.ts`, `planning-session.ts`, and `agent-kernel.ts` have been removed. The old host planner/session/kernel layer had no reason to survive after the Agentic OS host-kit cutover; keeping it would preserve a false second harness. xox production planning now enters through `agentic-os/xox-run-worker-adapter.ts`, then directly wires `xox-turn-intake-adapter.ts`, `xox-direct-answer-adapter.ts`, and `xox-agentic-os-host-kit.ts`.
- xox `turn-intake-resolver.ts` has been removed. Turn intake lane contract, schema, pending guard ordering, provider-unavailable fallback, and uncertain fallback now come from `@agentic-os/core`; xox `agentic-os/xox-run-worker-adapter.ts` only calls the thin adapter, and `agentic-os/xox-turn-intake-adapter.ts` supplies DB callbacks, runtime model callback, xox-specific prompt, and `goalFacts` sanitization.
- xox `direct-answer-runtime.ts` has been removed. Direct answer lane assistant-text-only validation, provider error handling, tool-call rejection, empty-output fail-closed behavior, and before-write cancellation now come from `@agentic-os/core`; xox `agentic-os/xox-run-worker-adapter.ts` only calls the thin adapter, and `agentic-os/xox-direct-answer-adapter.ts` supplies prompt, ambient context, runtime callback, DB writes, and localized product events.
...
- xox host observation bridge now consumes `@agentic-os/core` `createHostObservationBridge()`. Agentic OS owns canonical observation id/tool-call id mapping, host fallback hook invocation, and stable merge/dedupe; `apps/api/src/agent/agentic-os/xox-observation-adapter.ts` keeps only xox `AgentToolObservation` projection, legacy embedded `xoxObservation` recovery, product display copy fallback, and host key policy. `xox-agentic-os-host-kit.ts` and `xox-action-graph-adapter.ts` no longer keep local observation maps or fallback bridge helpers.
- xox action approval execution remains host product wiring, but root `apps/api/src/agent/approval-executor.ts` has been deleted. Confirm/cancel/edit route wiring, xox business execution, audit rows, memory candidate persistence, localized run events and continuation after action observations now live in `apps/api/src/agent/agentic-os/xox-action-approval-adapter.ts`.
