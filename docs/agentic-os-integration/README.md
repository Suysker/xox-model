...
- xox root `data-agent.ts`, `planning-context.ts`, and `runtime-intent-handlers.ts` have been removed. `data.query_workspace` business read execution now lives in `tool-executor.ts`, the concrete xox business tool boundary, and `PlannerContext` now lives beside xox action/read draft DTOs in `action-draft-builder.ts`. These are still xox peripherals; the deletion removes misleading root agent facade filenames.
