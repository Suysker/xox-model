...
Status: Draft (M123 sandbox runtime package)
...
- xox OpenAI Agents route now consumes `@agentic-os/runtime-openai-agents` directly from `runtime/runtime-adapter.ts`; local `runtime/openai-agents-adapter.ts` has been removed. Agentic OS owns OpenAI Agents SDK lifecycle, provider cleanup, SDK tool-call capture, runtime events, and SDK error redaction; xox keeps only provider settings, prompt compatibility, xox tool metadata fill, canonical tool call -> planner-step mapping, and localized event DTO mapping at the real provider selection boundary.
- xox OpenAI-compatible route now consumes `@agentic-os/runtime-openai-compatible` `runOpenAICompatibleRuntimeTurn()` from `runtime/openai-compatible-chat-adapter.ts`; Agentic OS owns request shaping, transport, stream parsing, provider turn normalization, tool-call normalization, provider artifact, and replay assistant message. xox adapter keeps only settings/prompt input mapping, localized event source bridging, and normalized provider call -> xox planner-step mapping.
- xox `runtime-planning-call.ts` now consumes `@agentic-os/runtime-openai-compatible` `runOpenAICompatibleRuntimePlanningRecovery()`; Agentic OS owns first/deferred/retry/missing-observation same-turn recovery orchestration. xox keeps only high-volume business budgets, tool catalog materialization callback, localized run events, evidence requirement callback, and `RuntimePlanResult` projection.
- xox `planner.ts` and `planning-session.ts` have been removed. The old host planner/session layer had no production source entrypoint after the Agentic OS host-kit cutover; keeping it would preserve a false second harness. xox production planning now enters through `agent-kernel.ts` and `xox-agentic-os-host-kit.ts`.
- xox `turn-intake-resolver.ts` has been removed. Turn intake lane contract, schema, pending guard ordering, provider-unavailable fallback, and uncertain fallback now come from `@agentic-os/core`; xox `agent-kernel.ts` only calls the thin adapter, and `agentic-os/xox-turn-intake-adapter.ts` supplies DB callbacks, runtime model callback, xox-specific prompt, and `goalFacts` sanitization.
- xox `direct-answer-runtime.ts` has been removed. Direct answer lane assistant-text-only validation, provider error handling, tool-call rejection, empty-output fail-closed behavior, and before-write cancellation now come from `@agentic-os/core`; xox `agent-kernel.ts` only calls the thin adapter, and `agentic-os/xox-direct-answer-adapter.ts` supplies prompt, ambient context, runtime callback, DB writes, and localized product events.
...
- xox still owns provider stream event projection, stream preview throttling, timeout/abort wiring, business request assembly, localized retry/status run-event copy, planning-boundary high-volume business tool policy, user/workspace provider settings, and provider tool call to xox planner-step mapping at the real adapter boundary. Provider turn execution, tool-call normalization, boundary validation, retry patching, deferred materialization orchestration, and missing-observation recovery now belong to Agentic OS.
- Obsolete local harness helper files are intentionally removed: `agent-run-engine.ts`, `turn-resolver.ts`, `agent-action-runtime.ts`, `context-engine/index.ts`, the former top-level `agentic-os-adapter.ts`, `planner.ts`, `planning-session.ts`, `turn-intake-resolver.ts`, `direct-answer-runtime.ts`, `clarification-resume.ts`, `loop-readiness-check.ts`, `memory-safety.ts`, `runtime/provider-failover-policy.ts`, `runtime/provider-request-shaper.ts`, `runtime/provider-probe.ts`, `runtime/tool-call-repair.ts`, `runtime/high-volume-tool-policy.ts`, `runtime/openai-agents-adapter.ts`, `runtime-plan-reader.ts`, `apps/api/src/agent/sandbox/*` runtime files, and provider runtime duplicates now owned by Agentic OS runtime packages.
