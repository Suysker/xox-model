...
- [x] ADR 0011 的 Layered Agent Transcript Disclosure 已落地：`AgentThreadState` 和 `AgentSendResponse` 返回 server-owned `transcriptNodes` 树，前端主 UI 不再消费 flat timeline；默认 transcript 显示用户气泡、assistant Markdown、work group / tool group / tool row 一行折叠、details/raw/confirmation 二级展开，pending 写入动作的可编辑确认卡挂在产生它的工具节点上；技术日志只通过显式 disclosure 查看，旧 provider stream preview 前端路径已删除
