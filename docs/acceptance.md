...
- [x] OpenClaw/Hermes-inspired provider runtime 兼容层已落地并迁入 Agentic OS package：`@agentic-os/runtime-openai-compatible` 拥有 ProviderModelRef、ProviderModelProfile、provider capability/thinking profile、request patch、replay policy、transcript replay、ToolSchemaNormalizer、provider payload sanitizer、ToolCallRepair、ProviderErrorClassifier 和 same-turn retry primitives；xox 只保留 ProviderRequestShaper adapter、ProviderProbe、stream event copy、高容量业务工具预算和 planner-step 映射；`provider-runtime.test.ts` 覆盖 DeepSeek/Qwen/Kimi/Gemini/vLLM/Anthropic 风格 profile、`tool_choice` shaping、thinking disable payload、schema cleanup、provider 输出修复、observation replay、错误分类和 retry policy
