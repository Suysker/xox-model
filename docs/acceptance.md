...
- [x] OpenClaw/Hermes-inspired provider runtime 兼容层已落地并迁入 Agentic OS package：`@agentic-os/runtime-openai-compatible` 拥有 ProviderModelRef、ProviderModelProfile、provider capability/thinking profile、request patch、replay policy、transcript replay、ToolSchemaNormalizer、provider payload sanitizer、ToolCallRepair primitives、ProviderToolCallNormalizer、ProviderToolCallStreamAssembler、ProviderBoundaryObservation、ProviderErrorClassifier、same-turn retry primitives 和 retry patch application；xox 已删除 `provider-failover-policy.ts`，只保留 ProviderRequestShaper adapter、ProviderProbe、stream event copy/preview throttle、timeout wiring、高容量业务工具预算、ReadDraft/中文文案包装、planner-step 映射和旧 DTO error 窄化；`provider-runtime.test.ts` 覆盖 DeepSeek/Qwen/Kimi/Gemini/vLLM/Anthropic 风格 profile、`tool_choice` shaping、thinking disable payload、schema cleanup、provider 输出修复、provider tool-call normalization、observation replay、provider boundary observation、错误分类和 retry policy
