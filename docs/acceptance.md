...
- [x] 主工作台保持 100% 正常尺寸；Agent Shell 默认是与页面内容同宽的底部半透明抽屉，可拖动高度、收起对话区并保留输入，页面底部会预留同等滚动空间避免遮挡最后内容；也可切换为右侧 Codex / VSCode 风格插件栏并拖动宽度，侧栏顶部使用图标级入口，底部常驻命令盒，不把抽屉工具栏挤进窄宽度；侧栏自动化级别使用浅色 Codex-style 权限 chip 弹出菜单，选项不重复“全力规划”文案，也不显示无实际附件/上下文能力的 `+` 入口
...
- [x] Agent 台展示当前 planner 来源、对话 id、工作区记忆列表和当前用户 / 工作区 provider 设置，并支持刷新、测试通过后保存、删除记忆或模型配置；已保存 provider key 只显示密码掩码，不回显真实 key；历史、记忆、模型入口有选中态，折叠状态点击会自动展开；历史和记忆面板会按抽屉剩余高度自适应，不固定成两行列表
...
- [x] OpenClaw/Hermes-inspired provider runtime 兼容层已落地并迁入 Agentic OS package：`@agentic-os/runtime-openai-compatible` 拥有 ProviderModelRef、ProviderModelProfile、provider capability/thinking profile、request patch、replay policy、transcript replay、ToolSchemaNormalizer、provider payload sanitizer、request shaper、provider probe、ToolCallRepair primitives、ProviderToolCallNormalizer、ProviderToolCallStreamAssembler、ProviderBoundaryObservation、ProviderErrorClassifier、same-turn retry primitives 和 retry patch application；xox 已删除 `provider-failover-policy.ts`、`provider-request-shaper.ts`、`provider-probe.ts`、`tool-call-repair.ts` 和 `high-volume-tool-policy.ts`，只保留 provider settings/key source、probe tool 名称与中文 copy、stream event copy/preview throttle、timeout wiring、`runtime-planning-call.ts` planning boundary 私有的高容量业务工具预算、ReadDraft/中文文案包装，以及 `openai-compatible-chat-adapter.ts` 内部的 xox planner-step 映射和旧 DTO error 窄化；`provider-runtime.test.ts` 覆盖 DeepSeek/Qwen/Kimi/Gemini/vLLM/Anthropic 风格 profile、`tool_choice` shaping、thinking disable payload、schema cleanup、provider 输出修复、provider tool-call normalization、observation replay、provider boundary observation、错误分类和 retry policy
- [x] Tool observation outcome classification 已迁入 `@agentic-os/core`：provider boundary、sandbox execution、action preview/result 的 `completed_valid / completed_invalid / failed_repairable / failed_terminal / pending_human / policy_blocked` 判断由 Agentic OS core 统一提供；xox `tool-observation-outcome.ts` 只保留 `@xox/contracts` 类型适配，架构守卫测试防止本地 classifier 分支回流
- [x] Action observation envelope 已迁入 `@agentic-os/core`：`action_preview` / `action_result` 的 model-readable payload 由 `buildActionPreviewObservation()` / `buildActionResultObservation()` 统一生成；xox `tool-observation-continuation.ts` 只保留中文展示文案、action row 映射、details 解析和业务 result summary，`action-graph-store.ts` 不再手写失败/策略阻断的 action-result JSON；`action-observation.test.ts` 和架构守卫覆盖该边界
- [x] Tool observation loop semantics 已迁入 `@agentic-os/core`：`parseToolObservationModelFacts()` 和 `is*ToolObservation()` helpers 统一识别 action、sandbox、provider boundary、tool supervisor failure、tool discovery 和 clarification observation；xox host kit 只保留业务 readable text 和 finalization policy，架构守卫防止重新直接 `JSON.parse(observation.modelContent)`
- [x] Tool observation continuation/finalizer prompt 已迁入 `@agentic-os/core`：`toolObservationContinuationSystemPrompt()` 统一维护 observation replay 后的安全回答边界、action preview/result 解释规则和内部 harness 信息隐藏规则；xox `prompt-registry.ts` 只注入平台身份与 host-specific provider 身份约束，本地 `tool-observation-finalizer.system.md` 已删除并由架构守卫防止回流
- [x] Provider observation turn message assembly 已迁入 `@agentic-os/runtime-openai-compatible`：`buildProviderToolObservationTurnMessages()` 统一生成 OpenAI-compatible planning/finalizer turn 的 system/prior/user-content/assistant-tool_calls/tool message 顺序、provider replay sanitation 和 runner-evidence 分离；`buildProviderToolObservationContinuationMessages()` 是 continuation wrapper；xox `runtime-planning-call.ts` 和 `tool-observation-continuation.ts` 不再直接调用 replay primitive 或手写 assistant/tool 配对
- [x] Progressive tool surface runtime 已迁入 `@agentic-os/core`：manifest builder、mixed Chinese/English tokenizer、tool search index、reranker、schema materializer、budget、discovery trace 和 visible/deferred/replayAllowed surface plan 由 Agentic OS 提供；xox 已删除 `apps/api/src/agent/tool-context-engine/*` 本地算法副本，只保留 `tool-surface-manifest.ts` 作为业务 manifest adapter 和旧 DTO 映射；`tool-context-engine.test.ts`、`tool-runtime.test.ts`、`sandbox-tool.test.ts` 和架构守卫覆盖该边界
- [x] Tool loop progress guardrails 已迁入 `@agentic-os/core`：`evaluateToolLoopGuardrails()` 统一检测 repeated failure、no-progress repair turn 和 executed write reapplied；xox `tool-runtime/tool-loop-guardrails.ts` 只保留 Row/DTO adapter，`approval-policy-composer.ts` 已删除并直接消费 core `composeAgentWriteApprovalPolicy()`；`tool-runtime.test.ts` 和架构守卫覆盖该边界
- [x] Host observation bridge 已迁入 `@agentic-os/core`：canonical `AgentObservation` 与 xox `AgentToolObservation` 之间的 id/tool-call 映射、fallback recovery 和 stable merge/dedupe 由 `createHostObservationBridge()` 提供；xox `xox-observation-adapter.ts` 已删除，DTO 投影、legacy embedded observation recovery、产品展示 fallback 和 key policy 已合并进 `xox-tool-observation-adapter.ts`，`xox-agentic-os-host-kit.ts` / `action-graph-store.ts` 不再维护本地 observation map 或 fallback bridge helper
...
- [x] Agent memory 按用户和工作区隔离，支持按 lane/status 查询、搜索、过滤、归档和候选晋升；长对话会生成同租户上下文摘要，并按 OpenClaw-style pre-compaction flush 写入 daily/session note，不再把压缩摘要直接塞进长期候选池
- [x] OpenClaw-first Memory Kernel v2 已落地并在 M168 收束到 Agentic OS：`@agentic-os/core` 承载 OpenClaw-derived flush plan、short-term promotion、MMR retrieval、citation、budget helpers、SaaS candidate policy、prompt injection eligibility、recall ranking、prompt lane budgets 和 query hashing；xox-local `packages/agent-memory-core` 和 `apps/api/tests/agent-memory-core.test.ts` 已删除；`memory_search / memory_get` 是一等只读工具；M140 后 active recall cache、timeout、circuit、prompt budget、citation 和 untrusted context rendering 由 `@agentic-os/core` `createAgentActiveMemoryRecallRuntime()` / `buildAgentActiveMemoryPromptPack()` 拥有，xox root `active-memory-recall.ts` 和 `memory/active-memory-subagent.ts` 已删除；M141 后 active recall lifecycle event drafts 也由 `@agentic-os/core` 拥有，xox `context-pack.ts` 不再实现 `onStarted/onSkipped/onCompleted/onInjected` 或手写 `memory_recall_*` 事件；`agent_memory_recall_signals` 记录 recall count、query diversity、recency 和 score；`agent_memory_dream_reports` 记录 reviewable dreaming sweep；Memory Center API/UI 展示 durable memories、daily notes、recall signals、dream reports、诊断和归档记忆
...
- [x] `sandbox_run_code` 已接入 Agentic OS first-class `AgentSandboxPort`：xox 只提供 workspace bundle、canonical manifest、SDK manifest、文件/后端外设和 `AgentSandboxExecutionResult` facts；Agentic OS 负责 sandbox observation、tool outcome、repair loop、final review 和 transcript/run-event projection。`SandboxBroker` 默认优先隔离 backend；本地无 Docker 的开发/测试需显式设置 `XOX_SANDBOX_BACKEND=local-script`，xox 仅在该显式选择下传递 unsafe-local opt-in。测试覆盖真实结构化输出、stdout/stderr、timeout、policy block、repairable sandbox failure replay、manifest-scoped metadata 和 sandbox event visibility。
...
- [x] Runtime plan reader facade 已删除：provider-neutral planner source 判定回到 `apps/api/src/agent/runtime/runtime-adapter.ts`，provider assistant text、空响应、认证失败、HTTP/network error 到只读 `ReadDraft` 的产品投影回到 `apps/api/src/agent/action-draft-builder.ts`；provider tool-call boundary failure 的 model-readable observation payload 已由 `@agentic-os/runtime-openai-compatible` 提供，xox 只包装产品文案
...
- [x] Agentic OS harness replacement 已落地：`apps/api/src/agent/agentic-os/xox-agentic-os-host-kit.ts` 是唯一 production harness 入口，旧 `agent-run-engine.ts`、`turn-resolver.ts`、`agent-action-runtime.ts` 和 `context-engine/index.ts` 已删除；xox 通过 Agentic OS ports 保留 context pack、tool gateway、completion evaluation、evidence review 和 action graph 业务语义；架构守卫测试防止退回本地 harness helper 或让 Action Graph 绕过确认卡策略
...
- [x] React Agent OS 支持 OpenAI-compatible provider token/tool-call chunk 级流式输出；服务端以 `stream: true` 调用兼容 Chat Completions provider，包含已有 tool observation 后的 continuation/final-answer turn，把脱敏截断后的 `provider_stream_*` 持久 run event 通过既有 thread state SSE 投影到前端实时预览；API 测试覆盖 fake SSE provider 的 chunk 聚合、observation 后续 turn 仍保持 streaming 和确认卡生成，web 测试覆盖前端聚合展示
