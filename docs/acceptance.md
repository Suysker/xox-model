...
- [x] ADR 0018 的 AgentRunEngine v2 单循环核心已落地：`apps/api/src/agent/agent-run-engine.ts` 是唯一 run-loop 入口，`turn-resolver.ts` 统一模型输出到 next-step 的分类，`agent-action-runtime.ts` 管理 Agent 发起写入的确认/自动执行/审计 observation 生命周期，`context-engine/index.ts` 成为 provider planning 的上下文入口；架构守卫测试防止退回 `goal-run-engine.ts`、Action Graph 直接执行确认卡或 Runtime Planning 绕开 ContextEngine
