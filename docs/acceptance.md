...
- [x] 主工作台保持 100% 正常尺寸；Agent Shell 默认是与页面内容同宽的底部半透明抽屉，可拖动高度、收起对话区并保留输入，页面底部会预留同等滚动空间避免遮挡最后内容；也可切换为右侧 Codex / VSCode 风格插件栏并拖动宽度，侧栏顶部使用图标级入口，底部常驻命令盒，不把抽屉工具栏挤进窄宽度，也不显示无实际附件/上下文能力的 `+` 入口
