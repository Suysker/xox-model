...
- [x] `sandbox_run_code` 已从 fake backend 升级为真实 manifest-scoped execution runtime：`SandboxBroker` 只能选择真实 backend，默认 `local-script` 子进程执行 Python/Node，`XOX_SANDBOX_BACKEND=docker` 可切 Docker；测试覆盖真实 `result.json` 解析、secret env 不透传、timeout、policy block、manifest fixture 不能满足 `requiresSandboxComputation`，Response Evaluator 只接受 `executionMode=executed + status=completed + exitCode=0 + structuredOutput` 的计算 evidence
