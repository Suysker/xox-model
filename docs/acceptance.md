...
- [x] OpenClaw-first Memory Kernel v2 已落地：`packages/agent-memory-core` 承载 OpenClaw-derived memory budget、flush plan、short-term promotion、citation 和 MMR retrieval 纯模块；`memory_search / memory_get` 是一等只读工具；`active-memory-recall.ts` 在 provider planning 前做 bounded/cache/circuit/budgeted active recall；`agent_memory_recall_signals` 记录 recall count、query diversity、recency 和 score；`agent_memory_dream_reports` 记录 reviewable dreaming sweep；Memory Center API/UI 展示 durable memories、daily notes、recall signals、dream reports、诊断和归档记忆
