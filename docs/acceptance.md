...
- [x] Agentic OS harness replacement 已落地：`apps/api/src/agent/agentic-os/xox-agentic-os-host-kit.ts` 是唯一 production harness 入口，旧 `apps/api/src/agent/agent-run-engine.ts` 已删除；xox 通过 Agentic OS ports 保留 turn resolution、action runtime、context、tool runtime、completion evaluation、evidence review 和 action graph 业务语义；架构守卫测试防止退回本地 `agent-run-engine.ts` / `goal-run-engine.ts` 或让 Action Graph 直接执行确认卡
