...
- [x] Runtime plan reader 已从 `planner.ts` 拆到 `apps/api/src/agent/runtime-plan-reader.ts`：该模块统一把 provider assistant text、空响应、认证失败、HTTP/network error 转成只读 `ReadDraft`，并负责 provider-neutral planner source 判定；provider tool-call boundary failure 的 model-readable observation payload 已由 `@agentic-os/runtime-openai-compatible` 提供，xox 只包装产品文案
