...
- [x] Host observation bridge 已迁入 `@agentic-os/core`：canonical `AgentObservation` 与 xox `AgentToolObservation` 之间的 id/tool-call 映射、fallback recovery 和 stable merge/dedupe 由 `createHostObservationBridge()` 提供；xox `xox-observation-adapter.ts` 只保留 DTO 投影、legacy embedded observation recovery、产品展示 fallback 和 key policy，`xox-agentic-os-host-kit.ts` / `action-graph-store.ts` 不再维护本地 observation map 或 fallback bridge helper
