...
- [x] Provider observation continuation message assembly 已迁入 `@agentic-os/runtime-openai-compatible`：`buildProviderToolObservationContinuationMessages()` 统一生成 OpenAI-compatible continuation 的 system/prior/user/assistant-tool_calls/tool message 顺序、provider replay sanitation 和 runner-evidence 分离；xox `tool-observation-continuation.ts` 不再直接调用 replay helper 或手写 assistant/tool 配对
