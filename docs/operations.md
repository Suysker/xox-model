...
- Tool call supervision 和 runtime event payload 也由 `@agentic-os/core` 提供。排查 provider tool call identity、参数安全化、工具目录外拦截、`tool_supervisor_failure` payload、工具执行 observation summary 或 inventory/tool-call runtime event payload 时，先看 Agentic OS `packages/core/src/tool-call-supervisor.ts` 和 `packages/core/test/tool-call-supervisor.test.ts`；xox `tool-runtime/tool-call-supervisor.ts` 只负责 planner step/result 映射和 run event persistence，`tool-execution-events.ts` 只保留 `@xox/contracts` adapter 与中文 summary。
