...
- Provider observation continuation message assembly 也由 `@agentic-os/runtime-openai-compatible` 提供。排查 observation replay 的 OpenAI-compatible message 顺序、assistant `tool_calls` 和 matching `tool` 消息、provider replay sanitation 或 runner-evidence 分离时，先看 Agentic OS `packages/runtime-openai-compatible/src/provider-transcript-replay.ts` 中的 `buildProviderToolObservationContinuationMessages()`；xox `tool-observation-continuation.ts` 只传入 same-thread log、用户消息、observations 和 provider profile，不要本地重新拼 assistant/tool replay 数组。
