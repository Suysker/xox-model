...
- `apps/api/src/agent/runtime/openai-agents-adapter.ts` 使用 OpenAI Agents SDK 的 `Agent / Runner / tool / OpenAIProvider`，SDK tool 只收集内部 plan step，不执行领域写入；`apps/api/src/agent/runtime/openai-compatible-chat-adapter.ts` 使用通用 Chat Completions `tools / tool_calls`，`tool_choice` 仅以 `auto` 发送或在 provider 不支持时省略，不使用 forced named choice，同时接受普通 assistant 文本作为只读回复。当 `LLM_PROVIDER` 不是 `rules` 时，常规 Agent 请求不会回退到规则/正则生成业务动作；只有 provider-native tool call 才能生成确认卡。真实 smoke 命令不允许无 key 运行。
