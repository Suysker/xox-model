...
| --- | --- | --- | --- | --- |
| ADR 0016 Manifest-Scoped Sandbox Tool | Manifest, capability, input bundle and direct-access isolation | ADR 0042 refines sandbox into a normal `ToolRuntime` execution target: no direct writes, but nested write tools may bridge through policy/approval/audit | Fake deterministic backend as proof of correctness for real calculations | Tool execution -> observation -> model continuation |
