...
本 ADR 进一步明确 Agent OS 的产品架构：系统不能只是“模型一次性看到所有工具，然后返回 tool call”。成熟的 Claude Code、Codex、OpenClaw 类系统更接近 harness agent：模型在受控运行壳内工作，外层 harness 负责租户隔离、上下文预算、记忆注入、工具选择、权限、确认、执行、审计、事件流和恢复。

当前实现已经具备真实 provider tool-call、确认卡、run event、SSE、memory、可编辑 pending action 和真实 smoke test，但仍存在关键差距：

- 工具目录目前仍是 flat catalog，provider 调用时会把完整工具集暴露给模型。
...
- `Capability Router` 负责根据用户指令、当前页面和权限选择业务能力域。
- `Tool Projection` 负责把完整工具注册表裁剪为本轮模型可见的小工具集。
...
本 ADR 暂不纳入 OpenAI 原生 `tool_search` 作为实现依赖。后续如果要引入，也只能作为 `Tool Projection` 的一种 runtime 优化，不能替代项目自己的权限、租户隔离和确认卡策略。
...
flowchart TB
  ...
  K --> CR[Capability Router<br/>识别任务域<br/>记账 / 模型 / 版本 / 分享 / 分析 / 管理]
  CR --> TP[Tool Projection<br/>按权限、页面、风险和任务域<br/>选择本轮小工具集]

  TP --> R[Runtime Adapter]
...
- `Tool Projection` 必须在 runtime 调用前完成，不能继续默认全量暴露工具。
...
sequenceDiagram
  ...
  Runtime->>Model: prompt + projected tools + context
...
- `Tool Broker` 应映射为 provider-neutral tool registry、capability router 和 tool projection，不再默认全量暴露工具。
...
- `capability-registry.ts`：声明能力域，例如 `ledger`、`workspace_draft`、`version`、`share`、`variance`、`data_query`。
- `capability-router.ts`：根据用户指令、当前页面、用户权限、workspace 状态选择候选能力域。
- `tool-projection.ts`：把完整工具注册表裁剪成本轮 runtime 可见工具集。
...
不再把完整工具集作为默认策略暴露给模型。每个工具必须有元数据：

- `capability`：所属能力域。
- `riskLevel`：`read | low | medium | high`。
- `requiresConfirmation`：是否必须确认。
...
2. `Capability Router` 识别任务域和风险。
3. `Tool Projection` 选择本轮候选工具，通常应控制在一个小集合。
4. `Runtime Adapter` 只把投影后的工具交给模型。
5. 模型返回 tool call 或 assistant text。
6. `Agent Kernel` 把 tool call 转成 read result 或 action request draft。

如果模型提出投影外工具或危险动作，kernel 必须拒绝或重新规划，不能临时扩大权限后直接执行。
...
- 架构图、模块边界、工具投影、memory、skills、SDK 边界写清。
...
### M3: Capability Router 与 Tool Projection
...
- 需要新增 capability router 和 tool projection 测试。
