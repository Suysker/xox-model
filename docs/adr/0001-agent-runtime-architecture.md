...
当前实现已经具备 TypeScript API、确认卡、可编辑待执行动作、租户内 memory、上下文摘要、OpenAI-compatible `tool_calls` 和显式 rules 降级路径。但这还不是成熟 Agent OS 架构：它把 provider 调用、planning、tool 映射、业务执行、memory 和 prompt 组织混在一个 API module 附近，缺少正式的 runtime 采用决策，也缺少对 OpenAI、Claude Code、OpenClaw 这类成熟架构的边界判断。
...
- 切换要求：后续换 OpenAI runtime 或任意 OpenAI-compatible provider 只能改 env / adapter，不改业务 tool 代码。
...
- `agent-kernel.ts`：一次用户消息触发一个 run。
...
- `apps/api/src/modules/agent.ts` 过大，混合了 runtime、planning、业务工具和 routes，需要拆成 `runtime / kernel / tools / routes`。
- `@openai/agents` 当前只是浅层 import，必须变成真正 adapter，或者在完成前从实现路径中移除避免误导。
...
- `LLM_PROVIDER=openai` 已有最小 OpenAI Agents SDK adapter，可用本地 fake OpenAI Chat Completions server 验证只读 tool call 和确认卡写入预览；后续仍需映射 SDK streaming、tracing 和 human-in-the-loop events。
- 文档中已完成的验收项需要拆成“当前可验证能力”和“下一阶段 Agent runtime maturity gate”，避免把 transitional implementation 当成最终架构。
...
- tool call、handoff/guardrail/tracing 能被 adapter 规范化为内部事件。
...
- `npm.cmd run test` 通过。
- 配置真实 OpenAI-compatible provider key 后，通过只读试算、多步骤确认写入、账号动作拒绝、memory 隔离 smoke test；默认 provider 可继续使用 DeepSeek。
...
- 需要重构现有 `apps/api/src/modules/agent.ts`。
- 短期内会出现 adapter 抽象和更多测试。
- OpenAI Agents SDK adapter、MCP 和 skills 都要按 feature flag 渐进引入；Claude Agent SDK 不进入实现计划。
