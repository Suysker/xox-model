...
> ADR0074 amendment (2026-07-11): current production uses the Agentic OS
> loop-owned inline model-turn architecture. Historical references below to a
> xox/provider Planner, Repair Planner, planner source, planning session, or
> runtime planning call are superseded. Current names are `AgentTurnContext`,
> `runtimeSource`, `agent_runs.runtime_source`, and ordinary `agent_turn`.
> Agentic OS owns continuation, evaluation repair, and terminal state; xox only
> supplies scoped business peripherals and product projection. Business
> `planning` fields inside the operating-model domain are unaffected.

本文件描述 `xox-model` 的目标 Agent OS 架构。正式 runtime 采用策略见 [ADR 0001](adr/0001-agent-runtime-architecture.md)，Harness Agent 分层架构见 [ADR 0002](adr/0002-harness-agent-architecture.md)，历史收敛记录见 [ADR 0003](adr/0003-xox-model-agent-os-target-architecture.md)，可靠性核心参考 [ADR 0004: Evaluator-Centered Harness Agent 架构](adr/0004-evaluator-centered-harness-agent.md) 和 [ADR 0018: AgentRunEngine v2 Single-Loop Harness Upgrade](adr/0018-agent-run-engine-v2-single-loop-harness.md) 的历史收束经验；当前 production harness 已由版本化 `@agentic-os/core` 的 Agentic OS loop 承担，xox 只保留业务 port、context pack、tool gateway、evaluator/evidence、sandbox 和 transcript/trace 投影。受控代码执行边界见 [ADR 0016: Manifest-Scoped Sandbox Tool](adr/0016-manifest-scoped-sandbox-tool.md)，统一工具/sandbox 运行面见 [ADR 0042: OpenClaw/Hermes Unified Tool Sandbox Runtime](adr/0042-openclaw-hermes-unified-tool-sandbox-runtime.md)：sandbox 只能在服务端生成的 manifest 内运行，不能直接访问 DB、secrets、internal HTTP 或领域服务；但 sandbox 内的 `agentic_os_sandbox.<tool_name>(...)` 必须桥回同一个 Tool Runtime Gateway，按同一套租户、权限、确认、导航、领域服务和审计链路执行。Memory Kernel 后续以 [ADR 0019: OpenClaw-First Memory Kernel v2](adr/0019-openclaw-first-memory-kernel-v2.md) 和 M168 为准：记忆不是日志池，必须按 working/session/semantic/procedural/episodic/diagnostic/archive 分层治理；OpenClaw-derived candidate policy、hybrid retrieval、promotion、citation、budget 和 compaction flush 已收束到 `@agentic-os/core`，xox 只保留 tenant-scoped store、Memory Center DTO 和业务 candidate 插件。

Frontend harness visibility now follows Agentic OS ADR0058/ADR0059. xox API projects `harnessUi` through Agentic OS server helpers, the web app renders it through `@agentic-os/ui-react` and `@agentic-os/ui`, and ordinary users default to the user run surface. Operator/developer surfaces are host-permission gated. xox keeps only product artifact rendering such as editable confirmation cards until Agentic OS exposes a generic artifact renderer slot; it must not fork the AG-UI reducer or rebuild generic harness run panels locally.

Agentic OS 抽取已经从 compatibility pilot 进入 production harness replacement：复杂目标路径现在由 `apps/api/src/agent/run-worker.ts` 取得 lease 后直接进入 `apps/api/src/agent/agentic-os/xox-agentic-os-host-kit.ts`，由版本化 `@agentic-os/core` 的 Agentic OS loop 承担单循环 harness。xox 继续拥有 evaluator、action graph、memory、sandbox、provider settings、tenant boundary 和业务工具语义，并把通用不变量反哺 Agentic OS。
...
- 受控沙箱执行：复杂预测、临时数据清洗、校验对账、敏感输入裁剪后的公式实验、常见文件格式处理和短期 artifact 生成可以走 `sandbox_run_code`。沙箱只接收当前租户的 manifest-scoped 最小化数据包；代码里可调用与 provider 工具同名、同参、同出参的 `agentic_os_sandbox.<tool_name>(...)`。读写都必须桥回同一个 Tool Runtime Gateway；写入动作按正常确认卡、自动化策略、领域服务和审计链路处理。如果 sandbox 内多个写入超过当前自动化等级，整次 sandbox run 生成一张聚合授权/确认，而不是逐个许可或一刀切拒绝。
...
Goal Interpreter
  ...
  -> Runtime Planner
  -> Action Graph Store
  -> Agentic OS ActionRuntime + xox action port
...
- `run-worker.ts` 在取得 lease 后直接委托 `agentic-os/xox-agentic-os-host-kit.ts` 负责把复杂目标推进成多轮 `plan -> confirm/execute -> observe -> evaluate -> repair`；本地 `agent-kernel.ts` facade 已删除。
...
- `automationLevel` 表示传给 Agentic OS 的执行授权程度，不表示 planner 推进力度。Planner/evaluator 始终全力推进；写入动作先落 `agent_action_requests`，再由 Agentic OS action lifecycle 判定自动执行或等待用户编辑/确认。
...
flowchart TB
  ...
  Graph --> Approval[Agentic OS ActionRuntime<br/>editable cards / policy / audit]
...
3. 写入 draft 先生成 server-owned action request 和可编辑确认卡；随后由 Agentic OS 判定 eligible 动作自动执行还是等待用户确认。
...
- 确认卡创建、编辑、确认、取消、执行状态更新、assistant message、run event 和审计当前由 Agentic OS action lifecycle 编排；route 只负责认证、HTTP DTO 序列化和 thread publish。Action graph step 的写入留在 `agentic-os/xox-action-graph-adapter.ts` 的 durable store boundary。
- 已确认 action 的业务执行位于 `apps/api/src/agent/tool-executor.ts`；Agentic OS 先做 action lifecycle/policy，再通过 xox action port 把当前 workspace/user 和 action payload 交给 executor 调用 workspace / ledger / share 模块。
  ...
  -> planning-session.ts
      ...
      -> runtime-intent-handlers.ts
          -> model-structure-action-drafts.ts
              ...
              -> xox-planned-items.ts types only
```
...
ADR 0015 的产品语义保留，但 ownership 已由 M167 修正：`automationLevel` 是传给 Agentic OS 的执行授权参数，不是 planner 推进力度。Agentic OS harness、provider planner、tool catalog、observation loop、evaluator 和 repair planner 都必须始终全力推进用户目标；自动化级别只在 action draft 已生成后影响 Agentic OS 是否可以自动执行。写入自动化策略由 Agentic OS core action lifecycle 提供，xox 不维护第二套 composer 或 authority resolver。

```text
manual  -> 读操作自动执行；所有写入保持可编辑 pending 确认卡
low     -> 读操作和低风险写入可自动执行；中/高风险写入等待确认
medium  -> 读操作、低风险写入、中风险写入可自动执行；高风险写入等待确认
high    -> 读操作、低风险写入、中风险写入可自动执行；高风险写入默认确认，除非 Agentic OS policy 明确允许
```

设计原则：

- 自动化是用户显式执行授权，不是模型自己决定权限，也不是降低或提高规划质量。
- 写入工具必须先生成 server-owned action request、导航事件、预览和 transcript 行；自动执行也必须走同一条 Agentic OS action lifecycle，只是由 Agentic OS 在 eligible action 上触发 `executeAction`。
- 自动执行和用户确认执行都必须调用同一个 xox action port、`assertActionExecutionAllowed`、domain service 和 audit 写入路径。
- 高自动化不是万能绕过。账号动作永远 forbidden；跨租户、锁账、派生分录、版本不可变、删除/覆盖/发布等高风险动作仍应停在确认或被 Agentic OS policy 阻止。
...
### `apps/api/src/agent/run-worker`

Agent OS 宿主运行边界，和具体 provider 实现解耦：

- `run-worker.ts`：持有后台 run controller、worker lease heartbeat、run completion finalization、run cancellation、进程重启恢复和队列 sweep；取得 lease 后只做 HostProfile/adapter wiring，直接进入 Agentic OS host kit 或 direct-answer lane。
- `agentic-os/xox-agentic-os-host-kit.ts`：xox HostProfile / HostAdapter wiring，连接 Agentic OS loop、xox tool gateway、action graph、evidence/evaluator、sandbox、memory 和 store ports。
- `action-graph-store.ts`：把 Agentic OS server materializer 输出映射成 xox action/read/status rows。
- `run-events.ts`：持久化 run 级 trace，并发布 thread state 刷新信号。
- `tool-policy.ts`：xox action kind、最低风险声明、导航、payload、租户/工作区和业务执行前校验；不决定自动执行。
...
- 复用计划：继续复用 `claimAgentRunLease`、`addRunEvent`、`buildThreadState` 和 `agentThreadEvents`；不新增并行业务执行路径。`run-worker.ts` 进入 Agentic OS host kit 前后在每次状态写入前刷新 lease。
...
读取、解释和预测可以自动执行；会改变草稿、版本、分享、账务、锁账状态、导入/导出或重置工作区的动作必须先生成 server-owned action request。随后由 Agentic OS action lifecycle 决定 eligible 写入自动执行还是等待用户编辑/确认。
...
- Agentic OS action lifecycle 判定自动执行或等待确认。
- 自动执行或用户确认后的执行和审计。
...
- `apps/api/scripts/agent-real-provider-smoke.ts`：CLI 和可复用 smoke runner。只负责创建临时 API harness、发起 Agent API 请求、断言结果和输出结构化摘要。
...
```text
npm smoke:agent
  -> apps/api/scripts/agent-real-provider-smoke.ts
...
- `run-worker.ts` 已直接委托 Agentic OS host kit 执行多轮 `plan -> action graph -> execute/confirm -> observe -> evaluate -> repair`；旧 `agent-kernel.ts`、`agent-run-engine.ts` 已删除，xox 不再维护第二套 production harness loop。
...
- `apps/api/src/agent/run-worker.ts` 已承接 durable queue/lease/cancel/recovery/finalization，并直接把 run 交给 Agentic OS host kit；下一阶段应继续删除 `agentic-os/xox-agentic-os-host-kit.ts` 中残留的 generic bookkeeping，但 routes 不应回收模型规划职责。
...
- ADR 0017 的本地 Tool Runtime Maturity Layer 已继续收敛：approval policy composer、tool-loop guardrails、effective inventory snapshot builder 和 tool-call supervisor runner 已迁入 `@agentic-os/core` / `@agentic-os/runtime-openai-compatible`；xox 已删除整个 `apps/api/src/agent/tool-runtime` 目录。Provider same-turn retry 的 recoverability、patch builder 和 patch application 也已迁入 `@agentic-os/runtime-openai-compatible`，xox 已删除 `apps/api/src/agent/runtime/provider-failover-policy.ts` 和 `apps/api/src/agent/runtime/high-volume-tool-policy.ts`，当前只在真实 planning boundary 保留 business action-draft callback、localized run-event copy、私有高容量业务工具预算、DTO mapping 和业务 action execution policy，不再保留本地 agent control plane。
...
- ADR 0019 的 OpenClaw-first Memory Kernel v2 已落地，并在 M168 从 xox-local package 收束到 Agentic OS core：记忆不再是日志池，而是 Agentic OS host kit 调用的上下文能力。OpenClaw-derived memory budget、flush plan、citation、MMR retrieval、short-term promotion scoring、SaaS candidate policy、prompt injection eligibility、recall ranking、prompt lane budgets 和 query hashing 由 `@agentic-os/core` 拥有；xox-local `packages/agent-memory-core` 已删除；`memory_search / memory_get` 是一等只读工具；`agent_memories` 继续承载 SaaS durable memory 和治理字段；`agent_memory_notes` 映射 OpenClaw daily notes；`agent_memory_recall_signals` 映射短期召回信号；`agent_memory_dream_reports` 映射 DREAMS/review surface。普通 prompt 只允许相关的可注入 working/semantic/procedural 记忆进入，candidate/diagnostic/episodic/archive/expired/superseded 默认不可注入；压缩摘要先写日记忆，不直接写长期候选。
...
9. 删除 `agent-kernel.ts` facade，由 `run-worker.ts` 在取得 lease 后直接进入 Agentic OS host kit：多轮 planning、执行/确认、observation、Completion Evaluator、Repair Planner。（已完成，旧本地 harness 文件已删除。）
