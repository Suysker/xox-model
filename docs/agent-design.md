...
- sandbox 内的工具说明必须来自同一个工具 manifest，而不是另一套私有协议。`docs/agent-tool-manifest.md` 是面向模型和 sandbox 的工具文档目标面，后续应由 `AGENT_TOOL_REGISTRY` 与 `buildToolManifests(...)` 生成或校验。`sandbox_run_code` 内应提供由 provider tool registry 生成的 `xox_sandbox.<tool_name>(...)` / JS camelCase SDK 函数；函数名、入参和返回契约尽量与模型看到的 tool calls 一致。只读函数在 manifest 授权的 observation/bundle 上执行，写入类函数只能返回受控 policy stop 并交回主循环走 provider tool + 确认卡，不能在 sandbox 内写业务数据。`xox_sandbox.rg(...)` 只能检索 manifest 授权的工具文档、同轮 observation 文档和安全输入文本，不能访问 repo、DB、env、日志或其他租户数据。
