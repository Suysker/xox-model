...
- ADR 0017 的本地 Tool Runtime Maturity Layer 已继续收敛：approval policy composer、tool-loop guardrails 和 effective inventory snapshot builder 已迁入 `@agentic-os/core` / `@agentic-os/runtime-openai-compatible`；xox 已删除本地 `tool-runtime/tool-loop-guardrails.ts` 与 `tool-runtime/effective-tool-inventory.ts`，当前只保留 business registry mapping、tool-call supervisor/action-draft adapter、run-event projection 和业务 action policy，不再保留本地 agent control plane。
