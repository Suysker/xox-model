...
团队成员新增/删除属于结构性草稿变更，不应让模型通过 `workspace_patch_config` 直接重写整个 `teamMembers` 数组。当前实现将其注册为两个专用 tool call：
...
这两类确认卡都显式导航到 `调模型 / 收入引擎`，用户可在执行前编辑载荷。确认执行前 `tool-policy.ts` 还会校验草稿至少保留 1 个成员，防止用户通过编辑确认卡把模型改到不可计算状态。
...
- `team_member_add` / `team_member_delete` 已作为结构性成员变更专用工具接入 tool catalog；服务端负责创建完整成员对象、删除目标校验、最后成员保护和确认卡预览。
