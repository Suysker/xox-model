...
describe('xox TypeScript API', () => {
  ...
  it('repairs a cross-domain goal and auto-executes eligible writes under high automation', async () => {
    ...
    await withFakeOpenAICompatibleProvider((body) => {
      ...
      expect(draftObservation.details).toContainEqual({ label: '股东 1 投资额', value: '65000 -> 1065000' })
      expect(draftObservation.result).toEqual({ revision: 2, workspaceName: '默认工作区' })
      expect(draftObservation.instruction).toContain('Do not say it is pending')
