...
describe('AgentConsole', () => {
  ...
  it('uses compact Codex-style chrome in the side panel', () => {
    ...
    try {
      ...
      expect(sideHeader?.querySelector('button[aria-label="收起对话"]')).toBeNull()
      expect(sideHeader?.querySelector('button[aria-label="展开对话"]')).toBeNull()
      ...
      expect(menuItems[3]?.textContent).toContain('低/中风险自动，高风险确认')
      expect(menuItems[3]?.innerHTML).not.toContain('truncate')
